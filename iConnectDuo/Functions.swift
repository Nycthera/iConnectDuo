...
func playNukeSound() {
    guard let url = Bundle.main.url(forResource: "nuke", withExtension: "mp3") else {
        print("nuke.mp3 not found in bundle")
        return
    }
    ...
    do {
        ...
        print("Playing nuke.mp3")
    } catch {
        print("Failed to play nuke.mp3:", error)
    }
}
        ...
        if let message = try? JSONDecoder().decode(Message.self, from: data) {
            ...
            case .responseUUID:
                if let uuid = message.payload, !peerDeviceIDs.contains(uuid) {
                    ...
                    peerUUID = uuid            // store in global
                    isConnected = true
                    requestPeerLocations()
                    
                    // Call getPeerMatchingData now that peerUUID is available
                    Task {
...
func geminiMatchImproved(completion: @escaping (String) -> Void) {
    ...
    let sanitizedPeer = peerResponse.trimmingCharacters(in: .whitespacesAndNewlines)
    let sanitizedOwn = myResponse.trimmingCharacters(in: .whitespacesAndNewlines)

    let prompt = """
    Evaluate if these two users match based on their responses:
    Peer: \(sanitizedPeer)
    Self: \(sanitizedOwn)
    Please respond with a simple 'Yes' or 'No' only.
    """

    let requestBody = GeminiRequest(
        contents: [GeminiRequest.Content(parts: [GeminiRequest.ContentPart(text: prompt)])]
    )
    ...
    let task = URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        do {
            if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
               ...
               let matchText = firstPart["text"] as? String {
                
                let result = matchText.trimmingCharacters(in: .whitespacesAndNewlines)
                print("Gemini Match Result:", result)
                ...
                print("Could not find match text in response")
            }
        } catch {
...
func getPeerMatchingData() async {
    let databaseId = "68a5944f0006ab9ec718"
    let collectionId = "68a59456002a3091735d"
    
    // Unwrap global peerUUID
    guard let stringPeerID = peerUUID else {
        print("peerUUID is nil, cannot fetch document")
        return
    }
    ...
    do {
        let document = try await databases.getDocument(
            databaseId: databaseId,
            collectionId: collectionId,
            documentId: stringPeerID
        )
        print("Document:", document.data)
        if let jsonData = try? JSONEncoder().encode(document.data),
           let jsonString = String(data: jsonData, encoding: .utf8) {
            peerResponse = jsonString
            print(peerResponse)
        }
        print(peerResponse)
    } catch {
...
class MatchNotifier: ObservableObject {
    
    func sendNotificationForSuccess() {
        let content = UNMutableNotificationContent()
        content.title = "MATCH!!!!!!!!!!"
        content.body = "YOU HAVE A MATCH!!!!!!"
        content.sound = .default
    ...
    func successNotification(result: String) {
        print("Checking result for notification:", result)
        if result.trimmingCharacters(in: .whitespacesAndNewlines) == "Yes" {
            print("Sending success notification")
            sendNotificationForSuccess()
        } else {
            print("Result is not 'Yes', no notification sent")
        }
    }
    
    func spamLoop(resultProvider: @escaping () -> String) {
        DispatchQueue.global().async {
            while true {
                let result = resultProvider()
                self.successNotification(result: "Yes")
                print("Loop running, current result:", result)
...
class HapticManager {
    ...
    func notification(type: UINotificationFeedbackGenerator.FeedbackType) {
        ...
        notificationGenerator.prepare() // prepare for next haptic
    }
    
    func impact(style: UIImpactFeedbackGenerator.FeedbackStyle) {
        impactGenerator.impactOccurred()
        impactGenerator.prepare() // prepare for next haptic
    }
}
